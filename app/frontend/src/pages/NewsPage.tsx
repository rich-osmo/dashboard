...
import { usePrioritizedNews, useRefreshPrioritizedNews, useDismissPrioritizedItem, useCreateIssue } from '../api/hooks';
import { TimeAgo } from '../components/shared/TimeAgo';
import { useFocusNavigation } from '../hooks/useFocusNavigation';
import { KeyboardHints } from '../components/shared/KeyboardHints';

const DAY_OPTIONS = [1, 14, 30] as const;
...
export function NewsPage() {
  ...
  const { containerRef } = useFocusNavigation({
    ...
    onDismiss: (i) => { if (items[i]) dismiss.mutate({ source: 'news', item_id: items[i].id }); },
  ...
  return (
    <div>
      ...
      {isLoading && <p className="empty-state">Loading prioritized news...</p>}
      {data?.error && <p className="empty-state">Error: {data.error}</p>}
      {!isLoading && !data?.error && items.length === 0 && (
        <p className="empty-state">
          {hiddenCount > 0
            ? `${hiddenCount} article${hiddenCount !== 1 ? 's' : ''} hidden below score ${minScore}`
            : `No news in the last ${days} day${days > 1 ? 's' : ''}`}
        </p>
      )}
            ...
            <div key={item.id} className="dashboard-item-row">
              ...
              >
                <div style={{ flexShrink: 0, paddingTop: '2px' }}>
                  {scoreBadge(item.priority_score)}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div className="dashboard-item-title">
                    {item.title}
                    {item.domain && (
                      <span className="news-item-domain" style={{ marginLeft: 'var(--space-xs)' }}>{item.domain}</span>
                    )}
                  </div>
                  <div className="dashboard-item-meta">
                    ...
                    {item.source_detail && item.source !== 'web' && (
                      <span> &middot; {item.source_detail}</span>
                    )}
                    {item.published_at && (
                      <span> &middot; <TimeAgo date={item.published_at} /></span>
                    )}
                  </div>
              ...
              <button
                ...
                onClick={() => dismiss.mutate({ source: 'news', item_id: item.id })}
                title="Mark as seen"
              >&times;</button>
            </div>
          );
      ...
      {hiddenCount > 0 && items.length > 0 && (
        <p className="empty-state" style={{ marginTop: 'var(--space-md)' }}>
          {hiddenCount} lower-priority article{hiddenCount !== 1 ? 's' : ''} hidden
